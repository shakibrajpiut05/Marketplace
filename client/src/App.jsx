...
const DASHBOARD_PAGES = ['seller-dashboard', 'buyer-dashboard', 'admin-dashboard', 'add-listing', 'verification']
    ...
    <nav className="bg-white border-b border-[#E5EAF0] sticky top-0 z-30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          {!isAuthenticated ? (<>
            <button onClick={() => onNavigate('auth')} className="px-4 py-1.5 text-sm font-medium text-[#374151] hover:text-[#0F1923]">Login</button>
            <button onClick={() => onNavigate('auth')} className="px-4 py-1.5 bg-[#5AC361] hover:bg-[#3EA646] text-white text-sm font-medium rounded-lg">Sign Up</button>
          </>) : (<>
            <button onClick={() => onNavigate(user?.role === 'admin' ? 'admin-dashboard' : `${user?.role}-dashboard`)} className="px-4 py-1.5 bg-[#5AC361] hover:bg-[#3EA646] text-white text-sm font-semibold rounded-lg">Dashboard</button>
        ...
        <div className="md:hidden border-t border-[#E5EAF0] bg-white px-4 py-3 flex flex-col gap-1">
          {[
            ...
            ...(user?.role === 'buyer' || !isAuthenticated
              ? [{ label: 'Post Requirement', page: 'buyer-dashboard' }]
              : []),
            ...(user?.role === 'seller' ? [{ label: 'Seller Portal', page: 'seller-dashboard' }] : []),
            ...(user?.role === 'admin' ? [{ label: 'Admin', page: 'admin-dashboard' }] : []),
          ].map((l) => (
            <button
              key={l.label}
              onClick={() => {
...
function App() {
  ...
  const [verificationToken, setVerificationToken] = useState(() => new URLSearchParams(window.location.search).get('verify-email') || '')
  ...
  const navigate = (nextPage, id) => {
    ...
    // Role-protect application pages at the client boundary.
    if (['seller-dashboard', 'add-listing'].includes(target) && user?.role !== 'seller') {
      target = user ? dashboardForRole(user.role) : 'auth'
    }
    ...
    // Authenticated users should not be left on the auth page.
    if (target === 'auth' && user) {
      target = dashboardForRole(user.role)
    }
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FB]">
      ...
      {page === 'email-verification' && <EmailVerificationPage token={verificationToken} onNavigate={navigate} />}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              ...
              {[
                {
                  title: 'Platform',
                  links: ['Browse Credits', ...(user?.role === 'seller' || user?.role === 'admin' ? [] : ['Post Requirement']), 'How It Works', 'Pricing'],
                },
