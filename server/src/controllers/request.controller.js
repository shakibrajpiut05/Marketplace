  ...
  try {
    ...
    if (!listing) {
      return res.status(404).json({
        success: false,
        message:
          "This credit listing is no longer available",
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Requested quantity must be a valid positive number",
      });
    }
...
export const getAdminPurchaseRequests = async (
  req,
  res
) => {
  try {
    ...
    console.error(
      "Get admin purchase requests error:",
      error
    );
...
export const getSellerPurchaseRequests = async (
  ...
  try {
    ...
    const sellerRequests = requests
      ...
      .map((request) => ({
        ...
        listing: {
          ...
          complianceYear:
            request.listingId.complianceYear,
          validTill:
            request.listingId.validTill,
        },
    ...
    console.error(
      "Get seller purchase requests error:",
      error
    );
    ...
    return res.status(500).json({
      success: false,
      message:
        "Failed to fetch seller purchase requests",
    });
  }
};
...
export const getBuyerPurchaseRequests = async (
  ...
  try {
    ...
    const buyerRequests = requests.map(
      (request) => ({
        _id: request._id,
    ...
    console.error(
      "Get buyer purchase requests error:",
      error
    );
    ...
    return res.status(500).json({
      success: false,
      message:
        "Failed to fetch buyer purchase requests",
    });
  }
};